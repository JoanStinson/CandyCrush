...
void L_gifFillSurface(uint8_t *pixels, L_GifFile* gif, int index) {/*fills pixel fields / consecutive*/
	...
	switch (imageAct.descriptor.imgPack.usesLocalColor) {/*selecting which color table to use*/
	case 0:/*global*/
	...
	for (i = 0; i < height * width; i++) {
		...
		if (imageAct.control.packField.alphaFlag && (alphaColor == imageAct.imageData[i])) {/*if color has index designed to be alpha*/
...
void L_gifFillSurfaceInterlace(uint8_t *pixels, L_GifFile* gif, int index) {/*fills pixel field / interlaced*/
...
int L_gifColorToInt(L_GifColor color) {/*color struct to int*/
...
void L_gifTextureRedraw(const uint8_t *pixels, const SDL_Rect* blitPos, SDL_Texture *dstTex) {/*redraw method, anything off the frame rect is made full alpha*/
...
void L_gifTextureOverlay(const uint8_t *pixels, const SDL_Rect* blitPos, SDL_Texture *dstTex) {/*overlay method, only frame rect is copied to surface*/
...
char L_gifPtIsInBox(int x, int y, const SDL_Rect *rect) {/*is this point in this Rect ?*/
	return ((x >= rect->x)
		&& (x < (rect->x + rect->w))
		&& (y >= rect->y)
		&& (y < (rect->y + rect->h)));
...
char L_gifBlit(CEV_GifAnim *anim) {/*redraw main texture*/
	...
	switch (dispMethod) {/*select disposal method*/
...
char L_gifAddModulo(int mode, int* val, int num) {/*incrément / décrément de val par modulo***VALIDE***/
...
char L_gifAddLim(int mode, int *val, int num) {/*incrément / décrément de val par limite***VALIDE***/
	switch (mode) {
	case 0:/*incremental*/
		if (*val + 1 < num)
			*val += 1;
...
void L_gifPicSelectNxt(CEV_GifAnim *anim) {/*selects next frame to be displayed*/
...
void L_gifFitBoxInto(SDL_Rect *adapt, const SDL_Rect *ref) {/*fits adapt into ref*/
	if (adapt->x < 0)
		adapt->x = 0;
...
char L_gifAnimInit(CEV_GifAnim * anim, L_GifFile* gif) {/*animation initialization*/
