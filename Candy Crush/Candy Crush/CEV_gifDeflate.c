...
typedef struct L_GifDicoEntry {/*dictionnary element*/
	int16_t prev;
	uint8_t value;
}
...
typedef struct L_GifDico {/*full lzw dictionnary*/
...
L_GifFile *L_gifLoadRW(SDL_RWops* file) {/*create gif handler*/
	...
	while (!endOfFile && !CEV_gifReadWriteErr) {
		...
		switch (dataType) {/*switch next data type*/
...
int L_gifExtFillRW(L_GifFile *gif, SDL_RWops* file) {/*fills misc extension blocks*/

	switch (SDL_ReadU8(file)) {/*switch file extension type*/
...
L_GifFile *L_gifCreate() {/*allocates and fills new structure*/
...
void L_gifHeaderInit(L_GifHeader *header) {/*header file init*/
...
void L_gifGceInit(L_GifGCE *gce) {/*gce file init*/
...
void L_gifLsdInit(L_GifLSD *lsd) {/*lsd file init*/
...
void L_gifComExtInit(L_GifComExt* comment) {/*comment extension init*/
...
void L_gifImgNewRW(L_GifFile *gif, SDL_RWops* file) {/*allocate and fills image information*/
	...
	if (gif->gce.used) {/*if graphic Control Extension was read before*/
	...
	if (gif->image[frameIndex].descriptor.imgPack.usesLocalColor) {/*if there is a local color table*/
...
void L_gifFileFree(L_GifFile *gif) {/*free all gif structure allocations*/
...
void L_gifDataFillRW(L_GifFile *gif, SDL_RWops* file) {/*read raw data sublock and send it thru lzw decompression*/
...
int L_gifHeaderFillRW(L_GifHeader* header, SDL_RWops* file) {/*fills header**/
...
int L_gifComExtFillRW(L_GifComExt *ext, SDL_RWops* file) {/*fills comment extension*/
...
int L_gifLsdFillRW(L_GifLSD* lsd, SDL_RWops* file) {/*fills Logical Screen Descriptor*/
...
int L_gifLsdPackFillRW(L_GifLSDpack* pack, SDL_RWops* file) {/*extract LSD's packed flags*/
...
int L_gifColorTabFillRW(unsigned int numOfColor, L_GifColorTable *colors, SDL_RWops* file) {/*fills color table*/
...
int L_gifGceFillRW(L_GifGCE* gce, SDL_RWops* file) {/*fills Graphic Control Extension*/
...
int L_gifGcePackFillRW(L_GifGCEPack* pack, SDL_RWops* file) {/*extract Graphic Control Extension's packed flags*/
...
int L_gifIdFillRW(L_GifID* gifid, SDL_RWops* file) {/*fills Image Descriptor*/
...
int L_gifIdPackFillRW(L_GifIDpack* IDpack, SDL_RWops* file) {/*extract Image Descriptor's packed flag*/
...
void L_gifBlockSkipRW(SDL_RWops* file) {/*skip useless subBlock in SDL_RWops*/
...
void L_gifLzw(void *codeStream, L_GifFile *gif, unsigned int lzwMinCode) {/*LZW Decompression*/
	...
	while (count < indexNum) {/*starts loop*/
		...
		if (actCodeValue < codeTable.actSize) {/**is CODE in the code table? Yes:**/
		...
		else {/**is CODE in the code table? No:**/
		...
		if (codeTable.actSize == (1 << (maskSize))) {/*if index reaches max value for actMask*/
...
uint16_t L_gifGetBitFieldValue16(void *data, unsigned int *bitStart, size_t maskSize) {/*16 bits value extraction*/
...
void L_gifDicoInit(L_GifDico * dico, unsigned int code) {/*init dictionnary*/
	int i;
